...
let chordTransitionStartTime = 0;
  ...
  if (activePage === 0) {
    ...
    if (sp2ViewMode === 'map') {
      ...
      if (activePane === 1) {
        ...
        const stationNames = ['الروحي', 'الأخلاقي', 'الترفيه (بطاقات ثلاثية الأبعاد)'];
        chordTitle = 'الوتر الثالث: التحضيرات • ' + (stationNames[typeof currentPrepStationIdx !== 'undefined' ? currentPrepStationIdx : 0] || 'الروحي');
        curStep = (typeof currentPrepStationIdx !== 'undefined' ? currentPrepStationIdx : 0) + 1;
        totalSteps = 3;
...
function closePage() {
  ...
  if (typeof stopAllPrepCardLoops === 'function') stopAllPrepCardLoops();
...
function stepToNextChord(targetIdx, startStep = 0) {
  ...
  if (isChordTransitioning && (Date.now() - chordTransitionStartTime < 1500)) return;
  isChordTransitioning = true;
  chordTransitionStartTime = Date.now();
  pendingChordStartStep = startStep;

  // 1. Return to the harp screen
  closePage();

  // 2. Wait for the harp to be visible, then pull the target string
  setTimeout(() => {
    ...
    // Guaranteed reset transition lock once target subpage has opened
    setTimeout(() => {
...
function advanceGlobalPresentation(direction) {
  // If a transition has been running for more than 1.5 seconds, auto-clear the lock
  if (isChordTransitioning) {
    if (Date.now() - chordTransitionStartTime > 1500) {
      isChordTransitioning = false;
    } else {
      return;
    }
  ...
  if (activePage === null) {
    if (direction > 0) {
      try { pluckHarpString(130.81, 0.75); } catch (e) { }
      openPage(0);
    }
    return;
  }
  ...
  if (activePage === 0) {
    if (direction > 0) {
      ...
      // Slide 8 (Station 4: الترابط - Christmas Video & Sports Padel Video in sequence)
      if (currentPptStep === 7) {
        if (!hasWatchedChristmasInSlide7 && !isChristmasVideoExpanded && !isBondingVideoExpanded) {
          ...
          closeBondingVideo(false);
          stepToNextChord(1, 0); // After both videos → go directly to Chord 1
          return;
        }
      }
      if (currentPptStep < 7) {
    ...
    if (direction > 0) {
      // ── NEXT ──
      if (sp2ViewMode === 'map') {
        // In map mode, pressing Next enters the currently highlighted node, or advances to Chord 3 if all completed
        if (currentGmLevel === 6 && sp2CompletedNodes.has(6)) {
          stepToNextChord(3, 0); // Advances to Chord 3 (التحديات)
        } else {
        ...
        if (activePane === 1) {
          ...
          // Slide 22: Preparations (3 Stations: 0: الروحي, 1: الأخلاقي, 2: الترفيه)
          if (currentPrepStationIdx === 0) {
            selectPrepStation(1, true);
          } else if (currentPrepStationIdx === 1) {
            selectPrepStation(2, true);
          } else {
            // Station 2 (الترفيه): flip through the 3 cards, then advance to Slide 24 Locations (pane 11)
            if (typeof prepCardsData !== 'undefined' && prepCardsData.length >= 3) {
              if (!prepCardsData[0].isFlipped) {
                togglePrepCard(0);
              } else if (!prepCardsData[1].isFlipped) {
                togglePrepCard(1);
              } else if (!prepCardsData[2].isFlipped) {
                togglePrepCard(2);
              } else {
      ...
      if (sp2ViewMode === 'map') {
        if (currentGmLevel <= 1) {
          stepToNextChord(1, 4); // Back to Chord 1 (Step 4: 2-days recap video)
        } else {
          currentGmLevel--;
        ...
        if (activePane === 10) {
          ...
          if (typeof selectPrepStation === 'function') {
            selectPrepStation(2, false);
          }
        } else if (activePane === 4) {
          // Inside Node 4 Preparations: reverse through Station 2 cards -> Station 1 -> Station 0 -> Map
          if (currentPrepStationIdx === 2) {
            if (typeof prepCardsData !== 'undefined' && prepCardsData.length >= 3) {
              if (prepCardsData[2].isFlipped) {
                togglePrepCard(2);
              } else if (prepCardsData[1].isFlipped) {
                togglePrepCard(1);
              } else if (prepCardsData[0].isFlipped) {
                togglePrepCard(0);
              } else {
                selectPrepStation(1, true);
              }
    ...
    if (direction > 0) {
      if (currentChallengeStep < 4) {
        ...
        stepToNextChord(4, 0); // Advances to Chord 4 (الدروس والمستقبل)
      }
    } else {
      if (currentChallengeStep > 0) {
        goToChallengeStep(currentChallengeStep - 1, true);
      } else {
        stepToNextChord(2, 27); // Back to Chord 2 (Day 2 Financials/Evaluation last step 27)
      }
    }
  } else if (activePage === 4) {
    if (direction > 0) {
      ...
      if (currentSp5SlideIdx === 4) {
        ...
        stepToNextChord(3, 4); // Back to Chord 3 (التحديات last step 4)
      }
    }
  }
}
...
document.addEventListener('keydown', e => {
  // Navigation key normalization (Space, Right Arrow, Left Arrow, Down Arrow, Up Arrow, Enter, PageDown, PageUp, Backspace)
  const isNext = (
  ...
  // 0. Disable key interception ONLY when user is actively focused and typing inside an editable field
  const isEditing =
  ...
  // Safety transition lock: auto-clear after 1.5s so presentation never gets permanently frozen
  if (isChordTransitioning) {
    if (Date.now() - chordTransitionStartTime > 1500) {
      isChordTransitioning = false;
    } else {
...
const imageSubpointsMap = {
  'assets/اعداد قادة.jpg': [
    {
      text: 'المحرك الرئيسي لاختيار المشروع',
      posClass: 'subpoint-top-right',
      variant: 'badge-variant-gold',
      icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><circle cx="12" cy="12" r="10"></circle><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"></polygon></svg>`
    }
  ],
  'اعداد قادة': [
    {
  ...
  'assets/session john and mira.jpg': [
    {
      text: 'كيفية التعامل مع الفئة العمرية',
      posClass: 'subpoint-top-right',
      variant: '',
      icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>`
    },
    {
      text: 'كيفية التعامل مع البنات',
      posClass: 'subpoint-bottom-left',
      variant: 'badge-variant-gold',
      icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>`
    }
  ],
  'session john and mira': [
    {
      text: 'كيفية التعامل مع الفئة العمرية',
      posClass: 'subpoint-top-right',
      variant: '',
...
function openStageZoom(src, tag, title, desc = '', subpoints = null) {
  const lightbox = document.getElementById('cinematic-lightbox');
  const img = document.getElementById('lightbox-img');
  const caption = document.getElementById('lightbox-caption');
  const subpointsContainer = document.getElementById('lightbox-floating-subpoints');
  ...
  if (caption) {
    caption.innerHTML = `
          ${tag ? `<span class="lightbox-tag">${tag}</span>` : ''}
          <h3 class="lightbox-title">${title}</h3>
          ${desc ? `<p class="lightbox-hint">${desc}</p>` : ''}
        `;
  }

  let effectiveSubpoints = subpoints;
  if (!effectiveSubpoints) {
    for (const [key, pts] of Object.entries(imageSubpointsMap)) {
      if (src && (src.includes(key) || key.includes(src) || decodeURIComponent(src).includes(key))) {
        effectiveSubpoints = pts;
        break;
      }
    }
  }
  ...
  if (subpointsContainer) {
    if (effectiveSubpoints && effectiveSubpoints.length > 0) {
      subpointsContainer.innerHTML = effectiveSubpoints.map((pt, idx) => `
            <div class="lightbox-floating-badge ${pt.posClass || (idx === 0 ? 'subpoint-top-right' : 'subpoint-bottom-left')} ${pt.variant || ''}" style="animation-delay: ${0.15 + idx * 0.18}s;">
              <span class="badge-icon-glow">${pt.icon || '<span class="badge-dot-glow"></span>'}</span>
              <span class="badge-text">${pt.text}</span>
            </div>
          `).join('');
...
function openCinematicZoom(src, title, tag = '', desc = '', subpoints = null) {
  openStageZoom(src, tag, title, desc, subpoints);
}
...
const eduCardsData = [
  ...
  },
  {
    ...
    title: 'لقائات لتبادل خبرات الخدمة',
    tag: '',
    desc: '',
    subpoints: []
  },
...
function openEduCardZoom(cardIdx) {
  ...
  if (!data) return;
  openCinematicZoom(data.img, data.title, data.tag, data.desc, data.subpoints);
...
function openDayVideo(dayNum) {
  ...
  if (dayNum === 1) {
    video.src = 'assets/first day viddd.mp4';
    video.poster = 'assets/first day raht image.jpg';
    video.onerror = function () {
      if (video.src.indexOf('vidddd') === -1) {
    ...
    video.src = 'assets/second day viddd.mp4';
    video.poster = 'assets/second day image.jpg';
    video.onerror = function () {
      if (video.src.indexOf('vidddd') === -1) {
...
const day1SectionsData = [
  {
    ...
    img: "assets/breakfast day 1.jpg",
  ...
  },
  {
    ...
    img: "assets/group photo day 1.jpg",
...
const day2SectionsData = [
  ...
  },
  {
    ...
    caption: "الانتهاء من نشاط حمام السباحة والاستحمام وتبديل الملابس"
  },
  {
    time: "06:00 - 06:30",
    dur: "٣٠ دقيقة",
    ...
    img: "assets/second day image.jpg",
...
function getNodeAndDayFromPane(paneIdx) {
  ...
  if (paneIdx === 4 || paneIdx === 11 || paneIdx === 7) return { node: 4, day: 1 };
...
function switchNodeDay(dayNum, playAudio = true) {
  ...
  if (currentGmLevel !== 3 && currentGmLevel !== 5 && currentGmLevel !== 6) return;
...
const gmLevelData = {
  ...
  4: {
    badge: 'المحطة الرابعة • التحضيرات',
    title: 'التحضيرات (الألعاب • حمام السباحة • الأشغال اليدوية)',
...
function openSp2Node(paneId, resetSubStep = true, playAudio = true) {
  ...
  // Always stop running auto loops when switching panes
  if (typeof stopAllPrepCardLoops === 'function') {
    stopAllPrepCardLoops();
  }

  // Ensure Day Toggle is strictly hidden on Slide 22 (pane 4)
  if (paneId === 4) {
    const container = document.getElementById('gm-day-toggle-container');
  ...
  if (resetSubStep) {
    if (paneId === 1) {
      ...
      if (typeof selectPrepStation === 'function') selectPrepStation(0, false);
      if (typeof resetAllPrepCards === 'function') resetAllPrepCards();
    } else if (paneId === 11) {
...
function openFullEvaluationSheet(event) {
  if (event) event.stopPropagation();
  const candidates = [
    'assets/full rate.png',
    'assets/full rate.jpg',
    'assets/full rate.jpeg',
    'assets/full rate.webp',
  ...
  function tryLoadCandidate(idx) {
    ...
    const img = new Image();
    img.onload = function () {
      openStageZoom(candidate, 'التقييم كامل • استمارة التقييم الميدانية الشاملة', 'استمارة التقييم الميدانية الكاملة', 'النموذج الشامل لتقييمات المشاركين وكافة الملاحظات الميدانية');
    };
...
// ══════════════ UNIFIED 3-STATION PREPARATIONS CONTROLLER (SLIDE 22: الروحي • الأخلاقي • الترفيه) ══════════════
let currentPrepStationIdx = 0; // 0: الروحي, 1: الأخلاقي, 2: الترفيه
...
function selectPrepStation(idx, playSound = true) {
  ...
  // If leaving Station 2, stop video/image auto loops
  if (idx !== 2 && typeof stopAllPrepCardLoops === 'function') {
    stopAllPrepCardLoops();
  }
...
// ══════════════ PREPARATION 3D FLIP CARDS & AUTO-LOOP MEDIA CONTROLLER (SLIDE 22) ══════════════
const prepCardsData = [
  {
    id: 0,
    title: 'الألعاب',
    folder: 'assets/card 1',
    curIdx: 0,
    timer: null,
    isFlipped: false,
    items: [
      { type: 'image', src: 'assets/card 1/57a91117-aaa6-4612-9824-04a869d3c4a2.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 1/5e8c613b-3a20-4537-83ae-7ab9a2bbe1d7.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 1/89e1c8dc-1b4a-43aa-81fe-66a7ab5870c3.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 1/9a59d62a-2892-4d52-ad82-fefd03e4379e.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 1/c30b4cb1-6574-4ecd-a599-2e6fcb2ea3d8.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 1/fb9341ff-2755-4881-b7ac-c1db69962fca.jpg', duration: 3500 },
      { type: 'video', src: 'assets/card 1/WhatsApp Video 2026-09-26 at 2.17.47 PM.mp4', duration: 5500 },
      { type: 'video', src: 'assets/card 1/WhatsApp Video 2026-09-26 at 2.17.49 PM.mp4', duration: 5500 },
      { type: 'video', src: 'assets/card 1/WhatsApp Video 2026-09-26 at 2.17.56 PM.mp4', duration: 6500 },
      { type: 'video', src: 'assets/card 1/WhatsApp Video 2026-09-26 at 2.17.59 PM.mp4', duration: 5500 },
      { type: 'video', src: 'assets/card 1/WhatsApp Video 2026-09-26 at 2.18.01 PM.mp4', duration: 5500 }
    ]
  },
  {
    id: 1,
    title: 'حمام السباحة',
    folder: 'assets/card 2',
    curIdx: 0,
    timer: null,
    isFlipped: false,
    items: [
      { type: 'image', src: 'assets/card 2/3d1065bb-ac66-4e2a-9cc5-9d6ace77ed29.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 2/44aa5cd2-f7ef-44ab-ad8f-11bbe1ec3b0a.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 2/b2ee5144-e8c1-4d47-a04b-b09738b02385.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 2/f6ec55cd-4810-44ff-86e5-53d470edfac4.jpg', duration: 3500 },
      { type: 'video', src: 'assets/card 2/WhatsApp Video 2026-09-26 at 2.19.21 PM.mp4', duration: 6500 },
      { type: 'video', src: 'assets/card 2/WhatsApp Video 2026-09-26 at 2.19.29 PM.mp4', duration: 5500 },
      { type: 'video', src: 'assets/card 2/WhatsApp Video 2026-09-26 at 2.19.58 PM.mp4', duration: 6500 },
      { type: 'video', src: 'assets/card 2/WhatsApp Video 2026-09-26 at 2.20.01 PM.mp4', duration: 5500 },
      { type: 'video', src: 'assets/card 2/WhatsApp Video 2026-09-26 at 2.20.06 PM.mp4', duration: 6500 }
    ]
  },
  {
    id: 2,
    title: 'الأشغال اليدوية',
    folder: 'assets/card 3',
    curIdx: 0,
    timer: null,
    isFlipped: false,
    items: [
      { type: 'image', src: 'assets/card 3/aa88ac71-fa24-44d0-8873-2860ca640d11.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 3/b0849800-9c2d-47f6-9241-90c0f1f631a4.jpg', duration: 3500 },
      { type: 'image', src: 'assets/card 3/fe185c03-911e-4b63-bc6b-51acebaf1b14.jpg', duration: 3500 },
      { type: 'video', src: 'assets/card 3/WhatsApp Video 2026-09-26 at 2.20.58 PM.mp4', duration: 5500 },
      { type: 'video', src: 'assets/card 3/WhatsApp Video 2026-09-26 at 2.21.01 PM.mp4', duration: 5500 },
      { type: 'video', src: 'assets/card 3/WhatsApp Video 2026-09-26 at 2.21.03 PM.mp4', duration: 5500 }
    ]
  }
];

function togglePrepCard(cardIdx, event) {
  if (event) event.stopPropagation();
  const cardEl = document.getElementById(`prep-card-${cardIdx}`);
  if (!cardEl) return;

  const cardData = prepCardsData[cardIdx];
  if (!cardData) return;

  const willBeFlipped = !cardEl.classList.contains('is-flipped');
  cardEl.classList.toggle('is-flipped', willBeFlipped);
  cardData.isFlipped = willBeFlipped;

  if (willBeFlipped) {
    if (typeof pluckHarpString === 'function') {
      pluckHarpString(cardIdx === 0 ? 329.63 : (cardIdx === 1 ? 392.00 : 440.00), 0.5);
    }
    renderPrepCardItem(cardIdx);
  } else {
    stopPrepCardLoop(cardIdx);
  }
}

function renderPrepCardItem(cardIdx) {
  const cardData = prepCardsData[cardIdx];
  if (!cardData || !cardData.items || !cardData.items.length) return;

  const holder = document.getElementById(`prep-media-holder-${cardIdx}`);
  const counter = document.getElementById(`prep-counter-${cardIdx}`);
  const badge = document.getElementById(`prep-type-badge-${cardIdx}`);
  const progress = document.getElementById(`prep-progress-${cardIdx}`);
  if (!holder) return;
  ...
  const item = cardData.items[cardData.curIdx];
  const total = cardData.items.length;

  if (counter) {
    counter.textContent = `${toArabicNum(cardData.curIdx + 1)} / ${toArabicNum(total)}`;
  }
  if (badge) {
    badge.textContent = item.type === 'video' ? '🎬 فيديو (GIF)' : '📷 صورة';
  }
  ...
  let playDuration = item.duration || 3500;

  if (item.type === 'image') {
    const img = document.createElement('img');
    img.src = item.src;
    img.alt = `${cardData.title} - ${cardData.curIdx + 1}`;
    img.style.width = '100%';
    ...
    img.onerror = function () {
      ...
      const altSrc = item.src.replace('assets/', '');
      if (img.src !== altSrc) {
        img.src = altSrc;
      }
    };
    ...
    requestAnimationFrame(() => {
      if (progress) {
        progress.style.transition = `width ${playDuration}ms linear`;
        progress.style.width = '100%';
      }
    });

    // Schedule next slide if card is active/flipped
    if (cardData.isFlipped) {
      cardData.timer = setTimeout(() => {
        stepPrepCard(cardIdx, 1);
      }, playDuration);
    }
    ...
    if (typeof video.play === 'function') {
      try {
        ...
        if (playPromise !== undefined) {
          playPromise.catch(e => console.log('Prep card video autoplay:', e));
        }
      } catch (e) { }
    }
    ...
    video.onloadedmetadata = function () {
      if (video.duration && !isNaN(video.duration) && video.duration > 1) {
        playDuration = Math.min(Math.max(video.duration * 1000, 3500), 9000);
      }
      if (progress) {
        progress.style.transition = `width ${playDuration}ms linear`;
        progress.style.width = '100%';
    ...
    setTimeout(() => {
      if (!cardData.timer && cardData.isFlipped) {
        if (progress) {
          progress.style.transition = `width ${playDuration}ms linear`;
          progress.style.width = '100%';
        }
...
function stepPrepCard(cardIdx, dir, event) {
  if (event) event.stopPropagation();
  const cardData = prepCardsData[cardIdx];
  if (!cardData || !cardData.items) return;

  const total = cardData.items.length;
  cardData.curIdx = (cardData.curIdx + dir + total) % total;
  renderPrepCardItem(cardIdx);
}

function stopPrepCardLoop(cardIdx) {
  const cardData = prepCardsData[cardIdx];
  if (!cardData) return;
  if (cardData.timer) {
    clearTimeout(cardData.timer);
...
function stopAllPrepCardLoops() {
  for (let i = 0; i < prepCardsData.length; i++) {
    stopPrepCardLoop(i);
  }
}

function resetAllPrepCards() {
  stopAllPrepCardLoops();
  for (let i = 0; i < prepCardsData.length; i++) {
    const cardEl = document.getElementById(`prep-card-${i}`);
    if (cardEl) {
      cardEl.classList.remove('is-flipped');
    }
    prepCardsData[i].isFlipped = false;
    prepCardsData[i].curIdx = 0;
  }
}

window.togglePrepCard = togglePrepCard;
window.stepPrepCard = stepPrepCard;
window.stopAllPrepCardLoops = stopAllPrepCardLoops;
window.resetAllPrepCards = resetAllPrepCards;
...
function navigateGameLevel(dir) {
  ...
  const paneOrder = [1, 2, 3, 6, 4, 11, 5, 8, 9, 10];
...
window.goToSp2Prep1Step = function () { };
window.handlePrep1CardTap = function () { };
window.goToSp2Prep2Step = function () { };
window.handlePrep2CardTap = function () { };
...
function goToSp2PptStep(step, playSound = true) {
  ...
  // Step 21: أماكن الخدمة على مدار يومين (pane 11)
  else if (step === 21) {
    openSp2Node(11, false, false);
...
// isEditorModeActive declared in navigation state at top of file
...
var CM_REGISTRY = [
  ...
  },
  {
    ...
    chordIdx: 1,
    stepIdx: 0,
    getter: () => (typeof rejectedCardsData !== 'undefined' && rejectedCardsData[0] ? rejectedCardsData[0].reason : 'المشروع يحتاج للمتخصصين اكثر من افراد الرهط'),
    setter: (v) => {
    ...
    getDefault: () => 'المشروع يحتاج للمتخصصين اكثر من افراد الرهط'
  },
  {
    ...
    chordIdx: 1,
    stepIdx: 0,
    getter: () => (typeof rejectedCardsData !== 'undefined' && rejectedCardsData[1] ? rejectedCardsData[1].title : 'رحلات الهايكينج'),
    setter: (v) => {
  ...
  },
  {
    ...
    chordIdx: 1,
    stepIdx: 0,
    getter: () => (typeof rejectedCardsData !== 'undefined' && rejectedCardsData[2] ? rejectedCardsData[2].reason : 'رفض من قائد العشيرة'),
    setter: (v) => {
      if (typeof rejectedCardsData !== 'undefined' && rejectedCardsData[2]) rejectedCardsData[2].reason = v;
    },
    getDefault: () => 'رفض من قائد العشيرة'
  },
  {
    id: 'c1_goal_label',
    category: 'chord1',
